...
module.exports = function (server) {
    ...
    server.del('/appointment', cancelAppointmentHandler);
     ...
     //get appointment APIs
    server.get('/appointment', getAppointmentHandler);
...
async function getAppointmentHandler(req, res, next) {
    try {
        const reschedule = require('../businessLogic/appointment').get;
        //calling openjet API
        const response = await reschedule(req.query);
        responseSend(req, res, 200, response, undefined);
        ...
        //error when parsing data
        logger.error("route.js, Handlers: getAppointmentHandler " + error.stack);
        responseSend(req, res, 400, undefined, error.message); 
        return next();
    }
}
